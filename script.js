...
function drawScope() {
  ...
  sCtx.fillStyle = "rgba(7,7,15,0.97)";
  sCtx.fill();
  [0.25, 0.5, 0.75, 1].forEach((f) => {
    sCtx.beginPath();
    sCtx.arc(cx, cy, R * f, 0, Math.PI * 2);
  ...
  if (trail.length > 1) {
    for (let i = 1; i < trail.length; i++) {
      ...
      sCtx.moveTo(a.x, a.y);
      sCtx.lineTo(b.x, b.y);
      sCtx.strokeStyle = `rgba(0,232,176,${t * 0.5})`;
      sCtx.lineWidth = 2 * t;
      sCtx.stroke();
    }
  }
  ...
  g.addColorStop(0, "rgba(0,232,176,0.3)");
  g.addColorStop(1, "transparent");
  sCtx.beginPath();
  sCtx.arc(d.x, d.y, 20, 0, Math.PI * 2);
  sCtx.fillStyle = g;
  sCtx.fill();
  sCtx.beginPath();
  sCtx.arc(d.x, d.y, 4.5, 0, Math.PI * 2);
  sCtx.fillStyle = "#00e8b0";
  sCtx.shadowColor = "#00e8b0";
  sCtx.shadowBlur = 14;
  sCtx.fill();
  sCtx.shadowBlur = 0;
...
(function drawViz() {
  if (analyser) {
    const W = vizCvs.offsetWidth * devicePixelRatio,
      H = vizCvs.offsetHeight * devicePixelRatio;
    vizCvs.width = W;
    vizCvs.height = H;
    const data = new Uint8Array(analyser.frequencyBinCount);
    analyser.getByteFrequencyData(data);
    vCtx.clearRect(0, 0, W, H);
    const bw = (W / data.length) * 2.1;
    let x = 0;
    for (let i = 0; i < data.length; i++) {
      const h = (data[i] / 255) * H * 0.86,
        t = i / data.length;
      const r = Math.round(t * 255),
        gg = Math.round(232 - t * 59),
        b = Math.round(176 - t * 117);
...
(function drawWave() {
  if (waveAnalyser) {
    const W = waveCvs.offsetWidth * devicePixelRatio,
      H = waveCvs.offsetHeight * devicePixelRatio;
    waveCvs.width = W;
    waveCvs.height = H;
...
const fileInput = qs("fileInput"),
  ...
  dlBtn = qs("dlBtn"),
  prog = qs("prog"),
  progFill = qs("progFill"),
  progTxt = qs("progTxt"),
  progPct = qs("progPct"),
  dot = qs("dot"),
  dotLbl = qs("dotLbl"),
  panOut = qs("panOut"),
  seekCtrl = qs("seekCtrl"),
  seekCur = qs("seekCur"),
  seekDur = qs("seekDur"),
  speedCtrl = qs("speedCtrl"),
  depthCtrl = qs("depthCtrl"),
  reverbCtrl = qs("reverbCtrl"),
  volCtrl = qs("volCtrl"),
  decayCtrl = qs("decayCtrl"),
  haasCtrl = qs("haasCtrl"),
  pitchCtrl = qs("pitchCtrl"),
  compThreshCtrl = qs("compThreshCtrl"),
  compRatioCtrl = qs("compRatioCtrl"),
  compKneeCtrl = qs("compKneeCtrl"),
  distCtrl = qs("distCtrl"),
  compToggle = qs("compToggle"),
  bassCtrl = qs("bassCtrl"),
  lowmidCtrl = qs("lowmidCtrl"),
  midCtrl = qs("midCtrl"),
  highmidCtrl = qs("highmidCtrl"),
  trebleCtrl = qs("trebleCtrl"),
  chorusMixCtrl = qs("chorusMixCtrl"),
  chorusRateCtrl = qs("chorusRateCtrl"),
  chorusDepthCtrl = qs("chorusDepthCtrl"),
  flangerMixCtrl = qs("flangerMixCtrl"),
  flangerRateCtrl = qs("flangerRateCtrl"),
  tremoloRateCtrl = qs("tremoloRateCtrl"),
  tremoloDepthCtrl = qs("tremoloDepthCtrl"),
  lpfCtrl = qs("lpfCtrl"),
  lpfQCtrl = qs("lpfQCtrl"),
  hpfCtrl = qs("hpfCtrl"),
  hpfQCtrl = qs("hpfQCtrl"),
  notchCtrl = qs("notchCtrl"),
  notchDepthCtrl = qs("notchDepthCtrl");

/* helpers */
function setStatus(on, txt) {
  dot.classList.toggle("on", on);
  dotLbl.classList.toggle("on", on);
  dotLbl.textContent = txt;
}
function fmtTime(s) {
...
function fmtFreq(f) {
  return f >= 1000 ? (f / 1000).toFixed(1) + "k Hz" : f + " Hz";
}
...
function syncLabels() {
  ...
  qs("bassVal").textContent =
    (bassCtrl.value >= 0 ? "+" : "") + bassCtrl.value + " dB";
  qs("lowmidVal").textContent =
    (lowmidCtrl.value >= 0 ? "+" : "") + lowmidCtrl.value + " dB";
  qs("midVal").textContent =
    (midCtrl.value >= 0 ? "+" : "") + midCtrl.value + " dB";
  qs("highmidVal").textContent =
    (highmidCtrl.value >= 0 ? "+" : "") + highmidCtrl.value + " dB";
  qs("trebleVal").textContent =
    (trebleCtrl.value >= 0 ? "+" : "") + trebleCtrl.value + " dB";
  qs("chorusMixVal").textContent = chorusMixCtrl.value + "%";
  qs("chorusRateVal").textContent =
    parseFloat(chorusRateCtrl.value).toFixed(1) + " Hz";
  qs("chorusDepthVal").textContent =
    parseFloat(chorusDepthCtrl.value).toFixed(1) + " ms";
  qs("flangerMixVal").textContent = flangerMixCtrl.value + "%";
  qs("flangerRateVal").textContent =
    parseFloat(flangerRateCtrl.value).toFixed(2) + " Hz";
  ...
  if (chorusWet && chorusDry) {
    const cm = chorusMixCtrl.value / 100;
    chorusWet.gain.value = cm;
    chorusDry.gain.value = 1 - cm * 0.5;
  }
  ...
  if (tremoloGain) {
    ...
    tremoloGain.gain.setTargetAtTime(1 - td * 0.7, audioCtx.currentTime, 0.01);
  }
...
compToggle.addEventListener("change", () => {
  if (compressor)
    compressor.threshold.value = compToggle.checked
      ? parseFloat(compThreshCtrl.value)
      : 0;
});
syncLabels();
...
document.querySelectorAll(".btn-preset").forEach((btn) => {
  btn.addEventListener("click", () => {
    ...
    if (!p) return;
    speedCtrl.value = p.speed;
    depthCtrl.value = p.depth;
    reverbCtrl.value = p.reverb;
    decayCtrl.value = p.decay;
    haasCtrl.value = p.haas;
    pitchCtrl.value = p.pitch;
    bassCtrl.value = p.bass;
    trebleCtrl.value = p.treble;
    lpfCtrl.value = p.lpf;
    hpfCtrl.value = p.hpf;
    chorusMixCtrl.value = p.chorusMix;
    flangerMixCtrl.value = p.flangerMix;
    tremoloDepthCtrl.value = p.tremoloDepth;
    distCtrl.value = p.dist;
...
function handleFile(file) {
  if (
    ...
    showToast("⚠ Pilih file audio (MP3, WAV, OGG)");
    return;
  }
  chipName.textContent = file.name;
  chipSz.textContent = fmtBytes(file.size);
  ...
  reader.onload = async (e) => {
    ...
    try {
      ...
      showToast("✓ File dimuat — siap diputar");
    } catch {
...
/* ── YouTube ── */
const ytInput = qs("ytInput"),
  ytBtn = qs("ytBtn"),
  ytStatus = qs("ytStatus");
function ytSetStatus(type, msg, spinner = false) {
  ytStatus.className = "yt-status" + (type ? " " + type : "");
  ytStatus.innerHTML =
    (spinner ? '<span class="yt-spinner"></span>' : "") + msg;
}
...
ytInput.addEventListener("paste", () =>
  setTimeout(() => {
    ...
    if (v.includes("youtube.com") || v.includes("youtu.be"))
      ytSetStatus("", "URL terdeteksi · tekan Enter atau klik Ambil");
  }, 50),
);
...
async function buildGraph() {
  ...
  highmidEq.frequency.value = 4000;
  highmidEq.Q.value = 1;
...
function scheduleMore() {
  ...
  const speed = parseFloat(speedCtrl.value),
    depth = parseFloat(depthCtrl.value);
  const dt = now - scheduleCtxTime,
    step = 0.02;
  schedulePhase += 2 * Math.PI * speed * dt;
  scheduleCtxTime = now;
  let t = now,
    ph = schedulePhase;
  while (t < until) {
    pannerNode.pan.setValueAtTime(Math.sin(ph) * depth, t);
    ph += 2 * Math.PI * speed * step;
    t += step;
  }
}
function startScheduler() {
  ...
  scheduleCtxTime = audioCtx.currentTime;
  scheduleMore();
  scheduleTimer = setInterval(scheduleMore, SCHEDULE_TICK);
}
...
function startScopeUpdate() {
  function tick() {
    ...
    if (!isSeeking) {
      ...
      seekCtrl.value = dispTime;
      seekCur.textContent = fmtTime(dispTime);
    }
    scopeRaf = requestAnimationFrame(tick);
  }
  scopeRaf = requestAnimationFrame(tick);
}
function stopScopeUpdate() {
...
async function play(offset) {
  ...
  isPlaying = true;
  sourceNode.onended = () => {
    if (isPlaying && !loopEnabled) stopPB();
  };
  startScheduler();
  startScopeUpdate();
...
function stopPB() {
  ...
  trail = [];
  curPan = 0;
  panOut.textContent = "PAN  0.00";
  panOut.classList.remove("on");
  playBtn.classList.remove("stop");
  playIco.textContent = "▶";
  playLbl.textContent = "Play 8D";
  setStatus(false, "ready");
}
...
/* ── Seek ── */
seekCtrl.addEventListener("mousedown", () => {
  isSeeking = true;
});
seekCtrl.addEventListener(
  "touchstart",
  () => {
    isSeeking = true;
  },
...
seekCtrl.addEventListener("input", () => {
  seekCur.textContent = fmtTime(parseFloat(seekCtrl.value));
});
...
seekCtrl.addEventListener("change", () => {
  if (isSeeking) return;
  commitSeek();
});
...
dlBtn.addEventListener("click", async () => {
  ...
  progTxt.textContent = "scheduling…";
  try {
    const dur = audioBuffer.duration,
    ...
    ohpf.type = "highpass";
    ohpf.frequency.value = parseFloat(hpfCtrl.value);
    ohpf.Q.value = parseFloat(hpfQCtrl.value);
    const olpf = off.createBiquadFilter();
    olpf.type = "lowpass";
    olpf.frequency.value = parseFloat(lpfCtrl.value);
    olpf.Q.value = parseFloat(lpfQCtrl.value);
    const onotch = off.createBiquadFilter();
    onotch.type = "peaking";
    onotch.frequency.value = parseFloat(notchCtrl.value);
    onotch.Q.value = 5;
    onotch.gain.value = -(notchDepthCtrl.value / 100) * 30;
    // EQ
    const ob = off.createBiquadFilter();
    ob.type = "lowshelf";
    ob.frequency.value = 100;
    ob.gain.value = parseFloat(bassCtrl.value);
    const olm = off.createBiquadFilter();
    olm.type = "peaking";
    olm.frequency.value = 400;
    olm.Q.value = 1;
    olm.gain.value = parseFloat(lowmidCtrl.value);
    const om = off.createBiquadFilter();
    om.type = "peaking";
    om.frequency.value = 1500;
    om.Q.value = 1;
    om.gain.value = parseFloat(midCtrl.value);
    const ohm = off.createBiquadFilter();
    ohm.type = "peaking";
    ohm.frequency.value = 4000;
    ohm.Q.value = 1;
    ohm.gain.value = parseFloat(highmidCtrl.value);
    const otr = off.createBiquadFilter();
    otr.type = "highshelf";
    otr.frequency.value = 8000;
    otr.gain.value = parseFloat(trebleCtrl.value);
    // Panner, dist, comp
    const opan = off.createStereoPanner();
    const odist = off.createWaveShaper();
    odist.curve = makeDistCurve(distCtrl.value / 100);
    odist.oversample = "4x";
    const ocomp = off.createDynamicsCompressor();
    ocomp.threshold.value = parseFloat(compThreshCtrl.value);
    ocomp.ratio.value = parseFloat(compRatioCtrl.value);
    ocomp.knee.value = parseFloat(compKneeCtrl.value);
    // Reverb
    const oconv = off.createConvolver();
    oconv.buffer = makeIR(
      off,
      Math.min(parseFloat(decayCtrl.value), 6),
      parseFloat(decayCtrl.value) / 2,
    );
    const odry = off.createGain(),
      owet = off.createGain(),
      omaster = off.createGain();
    const rm = reverbCtrl.value / 100;
    odry.gain.value = 1 - rm * 0.6;
    owet.gain.value = rm * 0.6;
    omaster.gain.value = volCtrl.value / 100;
    // wire
    src.connect(ohpf);
    ohpf.connect(olpf);
    olpf.connect(onotch);
    ...
    ocomp.connect(oconv);
    oconv.connect(owet);
    odry.connect(omaster);
    owet.connect(omaster);
    omaster.connect(off.destination);
    // schedule pan
    const spd = parseFloat(speedCtrl.value),
      dep = parseFloat(depthCtrl.value);
    const steps = Math.ceil(dur * 300);
    for (let i = 0; i <= steps; i++)
      opan.pan.setValueAtTime(
        Math.sin(2 * Math.PI * spd * (i / steps) * dur) * dep,
        (i / steps) * dur,
      );
    src.start(0);
    ...
    progPct.textContent = "95%";
    progTxt.textContent = "encoding…";
    const wav = toWav(rendered),
      url = URL.createObjectURL(wav);
    const a = document.createElement("a");
    a.href = url;
    a.download =
      (chipName.textContent.replace(/\.[^.]+$/, "") || "audio") + "_8D_Pro.wav";
    a.click();
    ...
    progTxt.textContent = "gagal: " + e.message;
...
function toWav(buf) {
  ...
  for (let i = 0; i < len; i++)
    for (let c = 0; c < nc; c++) {
      v.setInt16(
        off,
        Math.max(-32768, Math.min(32767, ch[c][i] * 0x7fff)),
        true,
      );
      off += 2;
    }
